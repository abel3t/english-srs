...
nojiApi.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response) {
      ...
      // Check if this is a private deck error (422 with specific message)
      const isPrivateDeckError = error.response.status === 422 && 
        error.response.data?.error?.title === 'This deck is private';
      
      if (isPrivateDeckError) {
        // Don't retry on private deck errors - let the caller handle it
